...
/**
 * Sling automation (MVP)
 *
 * Goals:
 * - Auto-create shifts when booking is paid
 * - Auto-update shifts when booking is edited
 * - Auto-cancel shifts when booking is cancelled/refunded
 * - Leave shifts unassigned/available (staff can claim in Sling)
 *
 * Integration modes:
 * - disabled: no-op
 * - webhook: POST booking payload to webhook
 * - direct: login with stored credentials, manage token, call Sling API
 */
...
add_action('init', function () {
    // Only if Action Scheduler exists (it ships with WooCommerce)
...
function roxy_eb_sling_job_sync_booking($booking_id, $reason = 'sync') {
    ...
    // Manual lock means "human owns Sling"
    if (($booking['sling_status'] ?? '') === 'manual') return;

    // If cancelled, do cancel flow.
...
function roxy_eb_sling_sync_shifts_for_booking($booking) {
    ...
    if ($mode === 'webhook') {
        // webhook is "fire and forget"; but we still want status in WP.
...
/**
 * Build a normalized payload for webhook mode and internal templating.
 */
function roxy_eb_sling_build_payload($booking, $action) {
    $settings = roxy_eb_get_settings();

    $doorsOpen = roxy_eb_mysql_to_dt($booking['doors_open_at']); // booking start time (doors open)
    ...
    // Sling shift timing:
    // Start 30 minutes before doors open, last 3 hours + any extra booked hours.
    ...
    $event_details = '';
    if (($booking['event_format'] ?? '') === 'movie') {
        $event_details = (string)($booking['movie_title'] ?? '');
    } else {
        $event_details = (string)($booking['live_description'] ?? '');
    }
    ...
    if (trim($notes_tpl) === '') {
        // Safety fallback: never send empty notes if templates were cleared.
...
function roxy_eb_sling_direct_sync($payload, $booking) {
    ...
    $guest_count = intval($booking['guest_count']);
    $desired = roxy_eb_sling_desired_roles($guest_count);
    ...
    foreach ($desired as $role) {
        ...
        $ids[$role] = [
            'shift_id' => $shift_id,
            'position_id' => $positionIds[$role] ?? null,
        ];
    ...
    // Update all existing shifts that we still want
    foreach ($desired as $role) {
        if (empty($ids[$role]['shift_id'])) continue;
    ...
    // Cancel any extra roles that are no longer needed
    foreach ($ids as $role => $data) {
        ...
        // Don't fail the whole sync if delete fails; just surface error next time.
        if (is_wp_error($res)) {
            return $res;
        }
        unset($ids[$role]);
...
function roxy_eb_sling_decode_shift_ids($raw) {
    ...
    // Back-compat: older versions stored a numeric list.
...
/**
 * Admin helper: test connection, log in, and attempt to resolve Location/Position numeric IDs.
 *
 * If list endpoints are unavailable, we fall back to manual numeric IDs.
 */
function roxy_eb_sling_admin_test_and_resolve($settings) {
    ...
    // Token must be configured (we do not support password login due to captcha).
    $token = roxy_eb_sling_get_token();
    if (is_wp_error($token)) return $token;

    // --- Reachability test (no auth required) ---
    // We intentionally hit /v1/health which is a lightweight endpoint in Sling's API spec.
    $base = rtrim($settings['sling_base_url'] ?? 'https://api.getsling.com', '/');
    // Normalize: allow admin to enter either https://api.getsling.com or https://api.getsling.com/v1
    $baseRoot = preg_replace('#/v1$#', '', $base);
    $healthUrl = $baseRoot . '/v1/health';

    $resp = wp_remote_get($healthUrl, [
        'timeout' => 15,
        'headers' => [
    ...
    if (is_wp_error($resp)) {
        return new WP_Error('sling_reach', 'Could not reach Sling API (health): ' . $resp->get_error_message());
    }
    $code = wp_remote_retrieve_response_code($resp);
    $body = wp_remote_retrieve_body($resp);
    if ($code < 200 || $code >= 300) {
    ...
    // --- Auth test (token required) ---
    // Use a spec-defined endpoint that requires auth, so we can detect expired/invalid tokens.
    $authCheck = roxy_eb_sling_api_request('GET', '/v1/shifts/current', null);
    if (is_wp_error($authCheck)) {
        return $authCheck;
    }

    // Mapping resolution is intentionally skipped for MVP because Sling "locations/positions" list endpoints
    // differ across accounts/plans and have caused 404s. Use manual IDs (or External IDs if Sling accepts them)
    // in settings.
    return [
        'ok' => true,
        'message' => 'Sling reachable and token valid. Mapping is manual in settings.',
    ];
}
...
function roxy_eb_sling_api_update_shift($shift_id, $body, $context = []) {
    // Sling may support PUT or PATCH; we use PUT for maximum compatibility.
...
function roxy_eb_sling_api_publish_shift($shift_id, $context = []) {
    // Publish a shift so employees can see/apply for it.
...
function roxy_eb_sling_api_list($type) {
    ...
    if (!in_array($type, ['locations','positions'], true)) {
        return new WP_Error('bad_list', 'Invalid list type');
    }
...
function roxy_eb_sling_api_request($method, $path, $body = null, $context = []) {
    ...
    // Authorization failures: notify admin so token can be updated.
    ...
    if (is_array($context) && !empty($context['action'])) {
        roxy_eb_sling_log_api($context, $path, $code, 'OK', $body, $out);
    }
    return $out;
}
...
function roxy_eb_sling_get_token() {
    ...
    if (empty($token)) {
        return new WP_Error('missing_token', 'Sling token not configured. Paste the Authorization token in settings.');
    }
    return $token;
}
...
function roxy_eb_sling_login($email, $password) {
    ...
    // Sling returns token in Authorization response header.
    $auth = '';
    if (is_array($headers)) {
        foreach ($headers as $k => $v) {
            if (strtolower($k) === 'authorization') {
                $auth = is_array($v) ? reset($v) : $v;
                break;
            }
        }
    ...
    if (empty($auth)) {
        return new WP_Error('missing_auth_header', 'Login succeeded but Sling did not return an Authorization header.');
    }
    return $auth;
}

// -----------------------------------------------------------------------------
// Secret storage (AES-256-CBC)
// -----------------------------------------------------------------------------
...
/**
 * Notify admin on Sling authorization failure (expired/invalid token).
...
function roxy_eb_sling_notify_failure($context, $http_code, $path, $respBody) {
    ...
    // Only notify for real automation actions
    ...
    // Rate limit: one email per booking+action per 6 hours
...
function roxy_eb_sling_notify_auth_failure($http_code, $path, $body) {
    ...
    // Send at most once per 6 hours per token.
    $cooldown = 6 * 3600;
    if ($last_hash === $token_hash && (time() - $last_ts) < $cooldown) {
        return;
    }
...
function roxy_eb_sling_log_api($context, $endpoint, $http_code, $message, $request_body = null, $response_body = null) {
    // Best-effort logging; failures should not break booking flow.
...
function roxy_eb_sling_admin_create_test_shift($settings) {
    if (($settings['sling_mode'] ?? 'disabled') !== 'direct') {
        return new WP_Error('not_direct', 'Set Sling Mode to “Direct API” to create a test shift.');
    }

    $map = roxy_eb_sling_get_mapping();
    if (is_wp_error($map)) return $map;

    // Tomorrow, 6pm-9pm local time (unpublished/planning by default).
    $tz = wp_timezone();
